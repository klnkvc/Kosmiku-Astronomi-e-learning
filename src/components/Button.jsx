...
const Button = ({ variant = 'primary', type, link, style, className, onClick, children }) => {
  ...
  if (variant === 'primary') {
    buttonClassName += 'bg-gradient-to-r from-based to-based-1 hover:from-based hover:to-shadowBased ';
  } else if (variant === 'dark') {
    buttonClassName += 'bg-black border-black hover:bg-zinc-900 hover:border-zinc-900 ';
  } else if (variant === 'outline') {
    buttonClassName += 'bg-transparant border border-based hover:bg-gradient-to-r hover:from-based hover:to-shadowBased text-black ';
  }
